...
Every v0.12.0 or v0.12.1 desktop build needs one manual install of the current v0.14.0 release. Both affected versions open to a launch-time 404, so Settings is inaccessible. The Intel v0.12.0 Mac build also cannot safely select the Apple silicon feed. Your workspace is stored separately and remains in place; normal in-app updates work from v0.12.2 onward.
:::
...
The OS installer itself is not Apple-notarized or Microsoft code-signed in v0.14.0. That is why Gatekeeper or SmartScreen may warn on first install even though the in-app updater has its own valid signature.
