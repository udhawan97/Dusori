...
| Ollama              | Companion            | `OLLAMA_MODEL`                             | Model-labeled output only after feature consent                     | Local model and license vary                                          |
...
AI is optional and consent is feature-specific. Depending on the feature, the disclosure may cover the query plus candidate metadata, accepted-source excerpts, a roadmap objective, current preferences, or typed instructions. The app labels model output and keeps deterministic ranking, synthesis structure, and prompt generation as fallbacks.
