...
Dated update paths, recap bounds, activity series, and the labels rendered from those series all share that UTC calendar key. The interface may localize the wording of a label, but it formats the stored key in UTC so another device timezone cannot move the activity to a different day. This is intentionally distinct from review scheduling, whose dates remain local to the learner.
