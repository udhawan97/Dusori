...
describe('research run memory', () => {
  ...
  it('keeps the original timestamp of an already-seen key while adding new ones', async () => {
    const storage = await topicStorage();
    await recordResearchRun(storage, 'azure-administration', { candidates: [{ key: 'wikipedia:1' }], providers: [], searchText: 'query' }, now);
  ...
  it('leaves dismissals untouched', async () => {
    ...
    await recordResearchRun(storage, 'azure-administration', { candidates: [{ key: 'wikipedia:1' }], providers: [], searchText: 'query' }, later);
  ...
  it('merges a competing hash-guarded write', async () => {
    ...
    await recordResearchRun(storage, 'azure-administration', { candidates: [{ key: 'wikipedia:42' }], providers: [], searchText: 'query' }, later);
  ...
  it('surfaces the retry-exhausted message when the write conflicts on all three attempts', async () => {
    ...
    await expect(
      recordResearchRun(storage, 'azure-administration', { candidates: [{ key: 'wikipedia:42' }], providers: [], searchText: 'query' }, later),
