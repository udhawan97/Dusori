...
- App pages: Research Desk with a visible query-first navigation field and five literal destinations (`Research`, `Sources`, `Learn`, `Map`, `Settings`). A submitted question creates or selects the topic, searches only disclosed providers, captures the diverse shortlist, reads quotable local text, and opens the resulting brief. Learning paths remain first-class without gating research. The source shelf stays one action away. The constellation is a secondary representation and always has a searchable Outline equivalent.
